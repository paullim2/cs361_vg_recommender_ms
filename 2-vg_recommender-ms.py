...
# function to receive data from web app, convert string value to list, run show_game_info function,
# and return value back to web app.
def display_game():
    input_string = request.json["input_string"]
    input = ast.literal_eval(input_string)
    recommended_game = recommend_game(input)
    return jsonify({"recommended_game": recommended_game})
