...
def main():
    ...
    vg_recommender_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    vg_recommender_socket.bind(('localhost', 8888))
    vg_recommender_socket.listen(1)

    print("Ready to receive connections...")

    while True:
        # Accept connection
        conn, addr = vg_recommender_socket.accept()
